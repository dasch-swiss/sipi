...
int main(int argc, char *argv[]) {
    ...
    int j2k_Clevels;
    sipiopt.add_option("--Clevels", j2k_Clevels,
    ...
    std::string j2k_Cblk;
    sipiopt.add_option("--Cblk", j2k_Cblk,
  ...
  */
    if (!sipiopt.get_option("--query")->empty()) {
        ...
        if (!sipiopt.get_option("--region")->empty()) {
            region = std::make_shared<Sipi::SipiRegion>(optRegion.at(0), optRegion.at(1), optRegion.at(2),
                                                        optRegion.at(3));
        }
        ...
        if (!sipiopt.get_option("--topleft")->empty()) {
            ...
            if (exif2 != nullptr) {
                unsigned short ori;
                if (exif2->getValByKey("Exif.Image.Orientation", ori)) {
                    orientation = static_cast<Sipi::Orientation>(ori);
                }
            }
        }
