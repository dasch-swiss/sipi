...
public:
    inline JpegError() :
        std::runtime_error("!! JPEG_ERROR !!") {}

    inline explicit JpegError(const char* msg) :
...
static boolean empty_file_buffer(j_compress_ptr cinfo) {
    ...
    size_t nn = 0;
    do {
        ssize_t tmp_n = write(file_buffer->file_id,
                              file_buffer->buffer + nn,
                              n);
        if (tmp_n < 0) {
...
static void term_file_destination(j_compress_ptr cinfo) {
    ...
    do {
        ...
        if (tmp_n < 0) {
            throw SipiImageError(thisSourceFile,
                                 __LINE__,
...
static void jpeg_file_dest(struct jpeg_compress_struct* cinfo, int file_id) {
    ...
    destmgr = (struct jpeg_destination_mgr*)malloc(
...
static int file_source_fill_input_buffer(struct jpeg_decompress_struct* cinfo) {
    ...
    size_t nbytes = 0;
    do {
        auto n = read(file_buffer->file_id,
                      file_buffer->buffer + nbytes,
                      file_buffer->buflen - nbytes);
        if (n < 0) {
            break; // error
        }
        if (n == 0)
...
static void file_source_skip_input_data(struct jpeg_decompress_struct* cinfo,
                                        long num_bytes) {
    if (num_bytes > 0) {
...
static boolean empty_html_buffer(j_compress_ptr cinfo) {
    auto* html_buffer = (HtmlBuffer*)cinfo->client_data;
    try {
        html_buffer->conobj->sendAndFlush(html_buffer->buffer,
                                          html_buffer->buflen);
    } catch (int i) {
 ...
 */
static void
jpeg_html_dest(struct jpeg_compress_struct* cinfo, shttps::Connection* conobj) {
    struct jpeg_destination_mgr* destmgr;
...
void SipiIOJpeg::parse_photoshop(SipiImage* img, char* data, int length) {
    ...
    while ((ptr - data) < length) {
        ...
        //
        // data processing
        datalen = ((unsigned char)*ptr << 24) | ((unsigned char)*(ptr + 1) <<
                16) |
        ...
        switch (id) {
            case 0x0404: {
                ...
                if (img->iptc == nullptr)
                    img->iptc = std::make_shared<SipiIptc>(
                        (unsigned char*)ptr,
                        datalen);
            ...
            case 0x0424: {
                ...
                // XMP data
                if (img->xmp == nullptr)
                    img->xmp = std::make_shared<SipiXmp>(ptr, datalen);
            }
            default: {
                // URL
...
bool SipiIOJpeg::read(SipiImage* img,
    ...
    try {
        ...
        close(infile);
        throw SipiError(thisSourceFile,
                        __LINE__,
                        "Error reading JPEG file: \"" + filepath + "\": " +
                        jpgerr.what());
    }
    ...
    if (no_cropping) {
        ...
        if ((size != nullptr) && (rtype != SipiSize::FULL)) {
            size->get_size(cinfo.image_width,
                           cinfo.image_height,
                           nnx,
                           nny,
                           reduce,
    ...
    while (marker) {
        if (marker->marker == JPEG_COM) {
            std::string emdatastr(reinterpret_cast<char*>(marker->data),
                                  marker->data_length);
            ...
            //
            auto* pos = static_cast<unsigned char*>(memmem(
                marker->data,
                marker->data_length,
                "Exif\000\000",
                6));
            if (pos != nullptr) {
                img->exif = std::make_shared<SipiExif>(
                    pos + 6,
                    marker->data_length - (pos - marker->data) - 6);
                uint16_t ori;
                if (img->exif->getValByKey("Exif.Image.Orientation", ori)) {
            ...
            //
            pos = (unsigned char*)memmem(marker->data,
                                         marker->data_length,
                                         "http://ns.adobe.com/xap/1.0/\000",
                                         29);
            if (pos != nullptr) {
                try {
                    char start[] = {'<', '?', 'x', 'p', 'a', 'c', 'k', 'e', 't',
                                    ' ', 'b', 'e', 'g', 'i', 'n',
                                    '\0'};
                    char end[] = {'<', '?', 'x', 'p', 'a', 'c', 'k', 'e', 't',
                                  ' ', 'e', 'n', 'd', '\0'};
            ...
            auto* pos = static_cast<unsigned char*>(memmem(
                ...
                12));
            if (pos != nullptr) {
                auto len = marker->data_length - (pos - (unsigned char*)marker->
                    data) - 14;
                icc_buffer = static_cast<unsigned char*>(realloc(
                    icc_buffer,
                    icc_buffer_len + len));
                Sipi::memcpy(icc_buffer + icc_buffer_len,
                             pos + 14,
                             (size_t)len);
                icc_buffer_len += len;
            }
    ...
    switch (colspace) {
        ...
        case JCS_YCCK: {
            throw SipiImageError(thisSourceFile,
                                 __LINE__,
                                 "Unsupported JPEG colorspace (JCS_YCCK)!");
        }
        case JCS_UNKNOWN: {
            throw SipiImageError(thisSourceFile,
                                 __LINE__,
                                 "Unsupported JPEG colorspace (JCS_UNKNOWN)!");
        }
        default: {
            throw SipiImageError(thisSourceFile,
                                 __LINE__,
    ...
    try {
        linbuf = (*cinfo.mem->alloc_sarray)((j_common_ptr)&cinfo,
                                            JPOOL_IMAGE,
                                            sll,
                                            1);
...
SipiImgInfo SipiIOJpeg::getDim(const std::string& filepath) {
    ...
    while (marker) {
        if (marker->marker == JPEG_COM) {
            ...
            // first we try to find the exif part
            //
            auto* pos = (unsigned char*)memmem(marker->data,
                                               marker->data_length,
                                               "Exif\000\000",
                                               6);
            if (pos != nullptr) {
...
void SipiIOJpeg::write(SipiImage* img,
                       const std::string& filepath,
    ...
    if ((params != nullptr) && (!params->empty())) {
        try {
            ...
            throw SipiImageError(thisSourceFile,
                                 __LINE__,
                                 "JPEG quality argument must be integer between 0 and 100");
        }
    ...
    try {
        ...
        jpeg_set_quality(&cinfo,
                         quality,
                         TRUE /* TRUE, then limit to baseline-JPEG values */);
    ...
    if (img->exif != nullptr) {
        ...
        if (buf.size() <= 65535) {
            ...
            auto exifchunk = shttps::make_unique<unsigned char[]>(
                buf.size() + start_l);
            Sipi::memcpy(exifchunk.get(), start, (size_t)start_l);
            Sipi::memcpy(exifchunk.get() + start_l,
                         buf.data(),
                         (size_t)buf.size());
    ...
    if (img->xmp != nullptr) {
        ...
        if ((!buf.empty()) && (buf.size() <= 65535)) {
            ...
            Sipi::memcpy(xmpchunk.get() + start_l,
                         buf.data(),
                         (size_t)buf.size());
            try {
                jpeg_write_marker(&cinfo,
    ...
    if ((img->icc != nullptr) || es.use_icc()) {
        ...
        unsigned char start[14] = {0x49, 0x43, 0x43, 0x5F, 0x50, 0x52, 0x4F,
                                   0x46, 0x49, 0x4C, 0x45,
                                   0x0}; //"ICC_PROFILE\000";
        size_t start_l = 14;
        ...
        for (unsigned int i = 0; i < n; i++) {
            ...
            if (n_nextwrite > n_towrite)
                n_nextwrite = n_towrite;
            Sipi::memcpy(iccchunk.get(), start, (size_t)start_l);
            Sipi::memcpy(iccchunk.get() + start_l,
                         buf.data() + n_written,
                         (size_t)n_nextwrite);
            try {
                jpeg_write_marker(&cinfo,
