 ...
 * it is also read into `SipiMetricsSnapshot` (src/ffi/cpp/metrics_snapshot.h — see
 * its "Inclusion rule" block for what belongs) by `sipi_metrics_snapshot`
 * (src/ffi/cpp/sipi_ffi.cpp) and mapped in server/rust/src/metrics.rs, from where the
 ...
 * `SipiMetricsSnapshot` (`src/ffi/cpp/sipi_ffi.cpp`) and are exported over OTLP by the
 * Rust shell — see `src/ffi/cpp/metrics_snapshot.h`'s inclusion rule for which fields
