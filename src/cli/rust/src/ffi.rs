//! The one C-ABI symbol `cli/rust` binds directly: `sipi_cli_main`.
//!
//! `cli/rust` does NOT route this through the `sipi` library — the two FFI
//! surfaces are disjoint (the server seam lives in `//src/server/rust`), so each
