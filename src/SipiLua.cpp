...
static int lua_cache_filelist(lua_State *L)
{
  ...
  if (top == 1) { sortmethod = std::string(lua_tostring(L, 1)); }
  lua_pop(L, top);
  std::shared_ptr<SipiCache> cache = server->cache();
...
static const luaL_Reg helper_methods[] = { { "filename_hash", lua_filenamehash_helper }, { nullptr, nullptr } };
//=========================================================================
...
static int SImage_get_exifgps(lua_State *L)
{
  ...
  char latref{ '\0' };
  if (!exif->getValByKey(std::string("Exif.GPSInfo.GPSLatitudeRef"), latref)) { latref = '\0'; }
  ...
  char longituderef{ '\0' };
  if (!exif->getValByKey(std::string("Exif.GPSInfo.GPSLongitudeRef"), longituderef)) { longituderef = '\0'; }
  ...
  char altituderef{ '\0' };
  if (!exif->getValByKey(std::string("Exif.GPSInfo.GPSAltitudeRef"), altituderef)) { altituderef = '\0'; }
  ...
  char speedref{ '\0' };
  if (!exif->getValByKey(std::string("Exif.GPSInfo.GPSSpeedRef"), speedref)) { speedref = '\0'; }
  Exiv2::Rational speed{ 0, 1 };
  if (!exif->getValByKey(std::string("Exif.GPSInfo.GPSSpeed"), speed)) { speed = { 0, 1 }; }

  char trackref{ '\0' };
  if (!exif->getValByKey(std::string("Exif.GPSInfo.GPSTrackRef"), trackref)) { trackref = '\0'; }
  Exiv2::Rational track{ 0, 1 };
  if (!exif->getValByKey(std::string("Exif.GPSInfo.GPSTrack"), track)) { track = { 0, 1 }; }

  char directionref{ '\0' };
  if (!exif->getValByKey(std::string("Exif.GPSInfo.GPSImgDirectionRef"), directionref)) { directionref = '\0'; }
  Exiv2::Rational direction{ 0, 1 };
  if (!exif->getValByKey(std::string("Exif.GPSInfo.GPSImgDirection"), direction)) { direction = { 0, 1 }; }

  char destlatref{ '\0' };
  if (!exif->getValByKey(std::string("Exif.GPSInfo.GPSDestLatitudeRef"), destlatref)) { destlatref = '\0'; }
  ...
  if (!exif->getValByKey(std::string("Exif.GPSInfo.GPSDestBearingRef"), destbearingref)) { destbearingref = '\0'; }
  Exiv2::Rational destbearing{ 0, 1 };
  if (!exif->getValByKey(std::string("Exif.GPSInfo.GPSDestBearing"), destbearing)) { destbearing = { 0, 1 }; }

  char destdistanceref{ '\0' };
  if (!exif->getValByKey(std::string("Exif.GPSInfo.GPSDestDistanceRef"), destdistanceref)) { destdistanceref = '\0'; }
  Exiv2::Rational destdistance{ 0, 1 };
  if (!exif->getValByKey(std::string("Exif.GPSInfo.GPSDestDistance"), destdistance)) { destdistance = { 0, 1 }; }
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    lua_pushnumber(L, (double)latitude[i].first / (double)latitude[i].second);
    // success - table1 - name - table2 - i - latitude
  ...
  for (int i = 0; i < 3; ++i) {
    lua_pushinteger(L, i);// success - table1 - name - table2 - i
    lua_pushnumber(L, (double)longitude[i].first / (double)longitude[i].second);
    // success - table1 - name - table2 - i - latitude
    lua_rawset(L, -3);// success - table1 - name - table2
  }
  lua_rawset(L, -3);// success - table1
  ...
  for (int i = 0; i < 3; ++i) {
    lua_pushinteger(L, i);// success - table1 - name - table2 - i
    lua_pushnumber(L, (double)altitude[i].first / (double)altitude[i].second);
    // success - table1 - name - table2 - i - latitude
    lua_rawset(L, -3);// success - table1 - name - table2
  }
  lua_rawset(L, -3);// success - table1
  ...
  for (int i = 0; i < 3; ++i) {
    lua_pushinteger(L, i);// success - table1 - name - table2 - i
    lua_pushnumber(L, (double)timestamp[i].first / (double)timestamp[i].second);
    // success - table1 - name - table2 - i - latitude
    lua_rawset(L, -3);// success - table1 - name - table2
  }
  ...
  for (int i = 0; i < 3; ++i) {
    lua_pushinteger(L, i);// success - table1 - name - table2 - i
    lua_pushnumber(L, (double)destlatitude[i].first / (double)destlatitude[i].second);
    // success - table1 - name - table2 - i - latitude
    lua_rawset(L, -3);// success - table1 - name - table2
  }
  lua_rawset(L, -3);// success - table1
  ...
  for (int i = 0; i < 3; ++i) {
    lua_pushinteger(L, i);// success - table1 - name - table2 - i
    lua_pushnumber(L, (double)destlongitude[i].first / (double)destlongitude[i].second);
    // success - table1 - name - table2 - i - latitude
    lua_rawset(L, -3);// success - table1 - name - table2
  }
  lua_rawset(L, -3);// success - table1
    ...
    if (lua_istable(L, 3)) {
      ...
      while (lua_next(L, 3) != 0) {
        ...
        if (key == std::string("Sprofile")) {
          std::set<std::string> validvalues{ "PROFILE0",
            "PROFILE1",
            "PROFILE2",
            "PART2",
            "CINEMA2K",
