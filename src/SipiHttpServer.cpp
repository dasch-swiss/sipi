...
static void send_error(Connection &conn_obj, Connection::StatusCodes code, const std::string &errmsg) {
  ...
  switch (code) {
    case Connection::BAD_REQUEST:http_err_name = "Bad Request";
      // log_err = false;
      break;

    case Connection::FORBIDDEN:http_err_name = "Forbidden";
      // log_err = false;
      break;

    case Connection::UNAUTHORIZED:http_err_name = "Unauthorized";
      break;

    case Connection::NOT_FOUND:http_err_name = "Not Found";
      // log_err = false;
      break;

    case Connection::INTERNAL_SERVER_ERROR:http_err_name = "Internal Server Error";
      break;

    case Connection::NOT_IMPLEMENTED:http_err_name = "Not Implemented";
      // log_err = false;
      break;

    case Connection::SERVICE_UNAVAILABLE:http_err_name = "Service Unavailable";
      break;

    default:http_err_name = "Unknown error";
      break;
  }
...
call_pre_flight(Connection &conn_obj,
  ...
  if ((preflight_info["type"] != "allow") &&
    ...
    std::ostringstream err_msg;
    err_msg << "The permission returned by Lua function " << pre_flight_func_name << " is not valid: "
            << preflight_info["type"];
    throw SipiError(__file__, __LINE__, err_msg.str());
  }
...
static std::unordered_map<std::string, std::string> check_file_access(
  ...
  if (luaserver.luaFunctionExists(pre_flight_func_name)) {
    pre_flight_info = call_pre_flight(conn_obj,
                                      luaserver,
...
static void knora_send_info(Connection &conn_obj, SipiHttpServer *serv, shttps::LuaServer &luaserver,
  ...
  } else if (actual_mimetype == "video/m4") {
    json_object_set_new(root, "internalMimeType", json_string(actual_mimetype.c_str()));
...
static void process_get_request(
  ...
  switch (service) {
    ...
    case SERVE_IIIF: {
      ...
      auto restriction_size =
          std::make_shared<SipiSize>(); // size of restricted image... (SizeType::FULL if unrestricted)
      ...
      if ((region->getType() == SipiRegion::FULL) && (size->getType() == SipiSize::FULL) && (angle == 0.0) &&
        ...
        switch (quality_format.format()) {
          case SipiQualityFormat::TIF: conn_obj.header("Content-Type", "image/tiff");
            break;
          case SipiQualityFormat::JPG: conn_obj.header("Content-Type", "image/jpeg");
            break;
          case SipiQualityFormat::PNG: conn_obj.header("Content-Type", "image/png");
            break;
          case SipiQualityFormat::JP2: conn_obj.header("Content-Type", "image/jp2");
            break;
          case SipiQualityFormat::PDF: conn_obj.header("Content-Type", "application/pdf");
            break;
          default: {}
        }
        try {
      ...
      if (cache != nullptr) {
        ...
        //!>
        std::string
            cachefile = cache->check(infile, canonical, true); // we block the file from being deleted if successfull
      ...
      try {
        img.read(infile,
                 ...
                 region,
                 size,
                 quality_format.format() == SipiQualityFormat::JPG,
                 serv->scaling_quality());
      ...
      if (quality_format.quality() != SipiQualityFormat::DEFAULT) {
        switch (quality_format.quality()) {
          case SipiQualityFormat::COLOR: img.convertToIcc(SipiIcc(icc_sRGB), 8);
            break; // for now, force 8 bit/sample
          case SipiQualityFormat::GRAY: img.convertToIcc(SipiIcc(icc_GRAY_D50), 8);
            break; // for now, force 8 bit/sample
          case SipiQualityFormat::BITONAL: img.toBitonal();
            break;
          default: {
