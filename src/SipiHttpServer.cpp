...
call_iiif_preflight(Connection& conn_obj,
    ...
    if (permission_return_val->type == LuaValstruct::STRING_TYPE) {
        ...
        try {
            ...
            std::ostringstream err_msg;
            err_msg << "The permission value returned by Lua function " << iiif_preflight_funcname <<
                " has no type field!";
            throw SipiError(__file__, __LINE__, err_msg.str());
        }
    ...
    if ((preflight_info["type"] != "allow") &&
        ...
        std::ostringstream err_msg;
        err_msg << "The permission returned by Lua function " << iiif_preflight_funcname << " is not valid: " <<
            preflight_info["type"];
        throw SipiError(__file__, __LINE__, err_msg.str());
    }
...
call_file_preflight(Connection& conn_obj, shttps::LuaServer& luaserver, const std::string& filepath) {
    ...
    if (permission_return_val->type == LuaValstruct::STRING_TYPE) {
        ...
        try {
            ...
            std::ostringstream err_msg;
            err_msg << "The permission value returned by Lua function " << file_preflight_funcname <<
                " has no type field!";
            throw SipiError(__file__, __LINE__, err_msg.str());
        }
        if (tmpv->type != LuaValstruct::STRING_TYPE) {
    ...
    if ((preflight_info["type"] != "allow") &&
        ...
        (preflight_info["type"] != "deny")) {
        std::ostringstream err_msg;
        err_msg << "The permission returned by Lua function " << file_preflight_funcname << " is not valid: " <<
            preflight_info["type"];
        throw SipiError(__file__, __LINE__, err_msg.str());
    }
...
static void serve_info_json_file(
    ...
    //
    if ((access["type"] == "login") || (access["type"] == "clickthrough") || (access["type"] == "kiosk") || (access[
        "type"] == "external")) {
        ...
        try {
            ...
            try {
                ...
                send_error(conn_obj,
                           Connection::INTERNAL_SERVER_ERROR,
                           "Pre_flight_script has login type but no tokenUrl!");
                return;
            }
            try {
            ...
            send_error(conn_obj,
                       Connection::INTERNAL_SERVER_ERROR,
                       "Pre_flight_script has login type but no cookieUrl!");
            return;
        }
    ...
    if (is_image_file) {
        ...
        if ((cache == nullptr) || !cache->
            getSize(access["infile"], width, height, t_width, t_height, clevels, pagenum)) {
    ...
    if (access(requested_file.c_str(), R_OK) == 0) {
        ...
        if (range.empty()) {
            ...
            conn_obj.header("Content-Disposition",
                            std::string("inline; filename=") + urldecode(params[iiif_identifier]));
            conn_obj.header("Content-Transfer-Encoding: binary");
            conn_obj.header("Last-Modified", timebuf);
