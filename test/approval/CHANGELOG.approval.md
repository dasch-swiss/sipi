...
|------|-----|-----------|-------|-------------------------|
| 2026-08-29 | sipi#797 | **Input fixture, not a golden:** `test/_test_data/images/unit/sample_with_icc.png` (`ImageEncodeBaseline.PngRoundTrip.approved.png` is unchanged and was re-verified byte-identical). | The fixture carried an incidental `Raw profile type exif` zTXt chunk that decompresses to a 3-byte `II*` with no IFD, which `Exif::parse` rejects. Under the repository's fatal-metadata contract (see `docs/src/development/error-model.md`) that made SIPI's own fixture an unreadable file, so the chunk was stripped with a byte-level chunk walker (no re-encode). `DEV-7056`. | `bazel test //test/approval/...` passes with the golden untouched — no re-approval. Before: 296786 bytes, chunks `IHDR, iCCP, zTXt("Raw profile type exif"), iTXt("SIPI:io.sipi.essentials"), IDAT×37, IEND, IEND`. After: 296740 bytes (−46, exactly the removed chunk's length+type+data+crc record), chunks `IHDR, iCCP, iTXt("SIPI:io.sipi.essentials"), IDAT×37, IEND, IEND` — all surviving chunks byte-identical, same order, same CRCs. Maintainer sign-off: I. Subotic, 2026-08-29. |
| 2026-07-26 | sipi#764 | `ImageEncodeBaseline.{JpegToTiffDownscaled, CmykTiffDownscaled, CielabTiffDownscaled, JpegRotatedDownscaledTiff}.approved.tif` + `.{JpegFullToJpegDownscaled, CmykTiffToJpeg, CielabTiffToJpeg, JpegRotatedToJpeg}.approved.jpg` + `.{JpegFullToPng, TiffToPng}.approved.png` (the same 10 downscale-through-`scale()` baselines) | **`SipiImage::scale()` resampler moved from `double` to fixed-point integer arithmetic.** Same separable filter (area-average downscale, 2-tap linear enlarge); only the accumulator type changes. Integer add is exact and associative, so the output is bit-identical regardless of SIMD lane width or architecture — the property that lets the kernel be vectorised (Highway, next commit) while keeping one golden set valid across x86-64 and aarch64. Pixel content unchanged; only the resampling rounding shifts ≤1 LSB from the previous `double` path. | `sipi compare` (absolute \|Δ\|) new-output-vs-previous-golden. **Lossless (TIFF/PNG):** max \|Δ\| **1–3 of 255**, avg 0.003–0.16 — the true double-vs-fixed-point rounding difference on high-frequency edges. **Lossy (JPEG):** max 8–26, avg 0.11–0.92 — that difference plus DCT re-encode, within the benign band the prior separable re-baseline (row below) accepted. Maintainer sign-off: PENDING PR review. |
| 2026-07-26 | sipi | `ImageEncodeBaseline.{JpegToTiffDownscaled, CmykTiffDownscaled, CielabTiffDownscaled, JpegRotatedDownscaledTiff}.approved.tif` + `.{JpegFullToJpegDownscaled, CmykTiffToJpeg, CielabTiffToJpeg, JpegRotatedToJpeg}.approved.jpg` + `.{JpegFullToPng, TiffToPng}.approved.png` (10 goldens — every baseline whose pipeline downscales through `scale()`) | **`SipiImage::scale()` rewritten to a separable area resampler.** The legacy path upsampled to an integer multiple ≥ source, bilinearly interpolated, then box-averaged back down — algorithmically wasteful (the `process`-tier `BM_ScaleHigh` ran 113–149 ms). The replacement is a two-pass separable resampler: area (box) averaging on a shrinking axis for an anti-aliased downscale, 2-tap linear interpolation on an enlarging axis, float intermediates. Same output dimensions/format/channels; only the resampling-filter response differs. `BM_ScaleHigh/256` 113 ms → 12.0 ms, `/1024` 149 ms → 32.1 ms (4–9×). ICC-only goldens are byte-identical (the change is scoped to resampling; `Icc::iccBytes()` untouched). | `sipi compare` (absolute \|Δ\|) decoded new-output-vs-old-golden. **Lossless outputs (TIFF/PNG)** — the true filter difference: max \|Δ\| **1–11 of 255**, avg 0.05–0.38, concentrated on high-frequency edges. **Lossy outputs (JPEG)** — filter difference plus DCT re-encode: max 15–23, avg 0.7–1.4, within the benign band the DEV-6563 re-baseline already accepted (a colorspace/channel bug would push avg into the dozens). Maintainer sign-off: PENDING PR review. |
