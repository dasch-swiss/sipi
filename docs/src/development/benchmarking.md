...
convention). The `parse`/`decode`/`encode`/`process` targets have all been
promoted into their modules (`//src/iiifparser/cpp/value_objects`,
`//src/format_handlers`, `//src/image_processing`). A `**/*_benchmark.cpp` glob
