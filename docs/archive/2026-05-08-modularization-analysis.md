...
> **Terminology note (revised 2026-05-18):** This document uses "master" / "service master" / "archival master" throughout, matching SIPI's vocabulary on the date it was written. Per [ADR-0009](../adr/0009-file-taxonomy.md) the terminology has since been replaced by a three-tier file taxonomy describing stages of a preservation pipeline — **Preservation File** (was: Archival master), **Service File** (was: Service master), **Access File** (previously implicit / unnamed). The architectural content of this analysis is unchanged; only the vocabulary has moved. When applying the findings to current work, translate the terms via the mapping above.
