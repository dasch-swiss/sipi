...
- **Mode flag on a single `convert` verb** (`sipi convert --create-service-master in.X out.Y`) — rejected. The flag-vs-subcommand discussion in the plan resolved this: the architectural seam between generic conversion and Service-File creation deserves a CLI verb, not a parameter on a shared verb. The bodies of the two operations share very little; surfacing that as the same verb misleads operators about the actual cost and effect of the flag.
...
- **`convert service-file` command lives in `src/cli/commands/convert_service_file.{h,cpp}`** (Phase 12 of DEV-6537's plan). Responsibilities: check prerequisites, read source via `readSource`, apply transformations (orientation, ICC convert per archival policy if applicable), compute pixel hash on post-transformation buffer, assemble `EssentialsFields` from observed source + computed hash + current `SipiImage` state, stamp the packet on the image via `essential_metadata(...)`, and call the format-handler writer. The writer emits the Essentials carrier because the packet is set on the image — no separate marker is needed.

- **Format-handler writers (pyramidal TIFF, JP2) gate Essentials emission on `essential_metadata().is_set()` on the incoming `SipiImage`.** No `SipiCompressionParams` key encodes the intent; the in-memory packet is the signal. JPEG/PNG/plain-TIFF writers never emit Essentials (per DEV-6379 Phase 5) — their format-handler write paths have no Essentials-carrier code at all.
