...
> **Superseded (2026-08, sipi#777):** the per-client rate limiter this ADR concerns was removed (wrong axis against distributed crawler swarms). The post-cache gate placement it established still holds for the surviving engine-side policy — the decode memory budget.
>
> **Reading the body below (historical).** It is preserved as the decision record as-made and is *not* a description of the current tree. Two things it says no longer hold: (1) *Throttling* now comprises **Admission** (shell-side, pre-dispatch) and the **Decode memory budget** (engine-side, post-cache), not the three named below — the Rate limiter and the Output size guard are both gone; (2) the code paths it names (`route_handlers/serve_iiif.cpp`, `throttling/client_id.{h,cpp}`, a `throttling/` module) were the then-planned target layout and were never built — the one surviving engine-side gate fires inline in `src/ffi/cpp/serve_image.cpp`. See `UBIQUITOUS_LANGUAGE.md` for the current *Throttling* terms.
