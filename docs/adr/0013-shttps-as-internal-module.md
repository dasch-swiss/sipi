...
- Re-homing schedule for shttps-internal utilities (`Hash` / `HashType` / `Parsing` / `Error` / `Global`) is relaxed from *Rust precondition* to *consistency cleanup*. The whole codebase is going to Rust anyway; those utilities will be re-implemented in the target language regardless of which side they live on in C++.
