...
- **Magic-string filepaths are replaced with typed variants** at the `read()` / `write()` surface. Write side: `OutputSink = std::variant<FilePath, StdoutSink, HttpSink, TeeSink>`. Read side: `InputSource = std::variant<FilePath, RangeSource>`. Internal handling of stdout-vs-HTTP-vs-file is unchanged for writes; only the API typing tightens. Resolves the ambiguity where `"HTTP"` was a special filename sentinel and the implicit assumption that read paths are always local POSIX files. The `HttpSink` and `RangeSource` alternatives carry opaque callbacks (no shttps types or storage-SDK types leak into `format_handlers/`). The variants live in `format_handlers/output_sink.h` and `format_handlers/input_source.h` per [ADR-0003](./0003-module-co-located-source-and-tests.md). Per-handler `RangeSource` integration (codec source-manager adapters: libtiff `TIFFClientOpen` callbacks, kakadu `kdu_compressed_source` subclass, libjpeg `jpeg_source_mgr`, libpng `png_set_read_fn`) is staged one PR per handler after `format_handlers/` is promoted to a Bazel package.
