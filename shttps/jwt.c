...
int jwt_set_alg(jwt_t *jwt, jwt_alg_t alg, const unsigned char *key, int len)
{
  ...
  if (alg < JWT_ALG_NONE || alg >= JWT_ALG_TERM) return EINVAL;
  ...
  default:
    if (!key || len <= 0) return EINVAL;

    jwt->key = malloc(len);
    if (!jwt->key) return ENOMEM;
...
jwt_alg_t jwt_get_alg(jwt_t *jwt) { return jwt->alg; }

int jwt_new(jwt_t **jwt)
{
  if (!jwt) return EINVAL;

  *jwt = malloc(sizeof(jwt_t));
  if (!*jwt) return ENOMEM;
...
static const char *get_js_string(json_t *js, const char *key)
{
  ...
  if (js_val) val = json_string_value(js_val);
...
static long get_js_int(json_t *js, const char *key)
{
  ...
  js_val = json_object_get(js, key);
  if (js_val) val = (long)json_integer_value(js_val);
...
static int jwt_sign_sha_hmac(jwt_t *jwt, BIO *out, const EVP_MD *alg, const char *str)
{
  unsigned char res[EVP_MAX_MD_SIZE];
  unsigned int res_len;

  HMAC(alg, jwt->key, jwt->key_len, (const unsigned char *)str, strlen(str), res, &res_len);
...
static int jwt_verify_sha_hmac(jwt_t *jwt, const EVP_MD *alg, const char *head, const char *sig)
{
  unsigned char res[EVP_MAX_MD_SIZE];
  ...
  HMAC(alg, jwt->key, jwt->key_len, (const unsigned char *)head, strlen(head), res, &res_len);
  ...
  if (len < 0) goto jwt_verify_hmac_done;
...
static int jwt_sign_sha_pem(jwt_t *jwt, BIO *out, const EVP_MD *alg, const char *str, int type)
{
  ...
  if (bufkey == NULL) SIGN_ERROR(ENOMEM);
  ...
  if (pkey == NULL) SIGN_ERROR(EINVAL);

  pkey_type = EVP_PKEY_id(pkey);
  if (pkey_type != type) SIGN_ERROR(EINVAL);

  mdctx = EVP_MD_CTX_create();
  if (mdctx == NULL) SIGN_ERROR(ENOMEM);

  /* Initialize the DigestSign operation using alg */
  if (EVP_DigestSignInit(mdctx, NULL, alg, NULL, pkey) != 1) SIGN_ERROR(EINVAL);
  ...
  if (EVP_DigestSignFinal(mdctx, NULL, &slen) != 1) SIGN_ERROR(EINVAL);

  /* Allocate memory for signature based on returned size */
  sig = alloca(slen);
  if (sig == NULL) SIGN_ERROR(ENOMEM);
  ...
  if (pkey_type != EVP_PKEY_EC) {
    ...
    if (ec_sig == NULL) SIGN_ERROR(ENOMEM);
    ...
    if (raw_buf == NULL) SIGN_ERROR(ENOMEM);
...
static int jwt_verify_sha_pem(jwt_t *jwt, const EVP_MD *alg, int type, const char *head, const char *sig_b64)
{
  ...
  if (sig == NULL) VERIFY_ERROR(EINVAL);

  bufkey = BIO_new_mem_buf(jwt->key, jwt->key_len);
  if (bufkey == NULL) VERIFY_ERROR(ENOMEM);

  /* This uses OpenSSL's default passphrase callback if needed. The
   * library caller can override this in many ways, all of which are
   * outside of the scope of LibJWT and this is documented in jwt.h. */
  pkey = PEM_read_bio_PUBKEY(bufkey, NULL, NULL, NULL);
  if (pkey == NULL) VERIFY_ERROR(EINVAL);
  ...
  if (pkey_type == EVP_PKEY_EC) {
    ...
    if (ec_sig_r == NULL || ec_sig_s == NULL) VERIFY_ERROR(EINVAL);
    ...
    sig = malloc(slen);
    if (sig == NULL) VERIFY_ERROR(ENOMEM);
  ...
  mdctx = EVP_MD_CTX_create();
  if (mdctx == NULL) VERIFY_ERROR(ENOMEM);

  /* Initialize the DigestVerify operation using alg */
  if (EVP_DigestVerifyInit(mdctx, NULL, alg, NULL, pkey) != 1) VERIFY_ERROR(EINVAL);
...
static int jwt_verify(jwt_t *jwt, const char *head, const char *sig)
{
  ...
  case JWT_ALG_HS512:
    ...
    /* RSA */
  case JWT_ALG_RS256:
    return jwt_verify_sha_pem(jwt, EVP_sha256(), EVP_PKEY_RSA, head, sig);
  case JWT_ALG_RS384:
    return jwt_verify_sha_pem(jwt, EVP_sha384(), EVP_PKEY_RSA, head, sig);
  case JWT_ALG_RS512:
    return jwt_verify_sha_pem(jwt, EVP_sha512(), EVP_PKEY_RSA, head, sig);

    /* ECC */
  case JWT_ALG_ES256:
    return jwt_verify_sha_pem(jwt, EVP_sha256(), EVP_PKEY_EC, head, sig);
  case JWT_ALG_ES384:
    return jwt_verify_sha_pem(jwt, EVP_sha384(), EVP_PKEY_EC, head, sig);
  case JWT_ALG_ES512:
    return jwt_verify_sha_pem(jwt, EVP_sha512(), EVP_PKEY_EC, head, sig);
...
static int jwt_verify_head(jwt_t *jwt, char *head)
{
  ...
  if (ret) goto verify_head_done;

  if (jwt->alg != JWT_ALG_NONE) {
    /* If alg is not NONE, there may be a typ. */
    val = get_js_string(js, "typ");
    if (val && strcasecmp(val, "JWT")) ret = EINVAL;

    if (jwt->key) {
      if (jwt->key_len <= 0) ret = EINVAL;
    } else {
      jwt_scrub_key(jwt);
    }
  } else {
    /* If alg is NONE, there should not be a key */
    if (jwt->key) { ret = EINVAL; }
  }
...
int jwt_decode(jwt_t **jwt, const char *token, const unsigned char *key, int key_len)
{
  ...
  *jwt = NULL;
   ...
   * header. */
  ret = jwt_new(&new);
  if (ret) { goto decode_done; }

  /* Copy the key over for verify_head. */
  if (key_len) {
    new->key = malloc(key_len);
    if (new->key == NULL) goto decode_done;
...
{
  if (!jwt || !grant || !strlen(grant) || !val) return EINVAL;

  if (get_js_string(jwt->grants, grant) != NULL) return EEXIST;

  if (json_object_set_new(jwt->grants, grant, json_string(val))) return EINVAL;
...
{
  if (!jwt || !grant || !strlen(grant)) return EINVAL;

  if (get_js_int(jwt->grants, grant) != -1) return EEXIST;

  if (json_object_set_new(jwt->grants, grant, json_integer((json_int_t)val))) return EINVAL;
...
int jwt_add_grants_json(jwt_t *jwt, const char *json)
{
  ...
  if (json_is_object(js_val)) ret = json_object_update(jwt->grants, js_val);
...
int jwt_del_grant(jwt_t *jwt, const char *grant) { return jwt_del_grants(jwt, grant); }

static void jwt_write_bio_head(jwt_t *jwt, BIO *bio, int pretty)
{
  BIO_puts(bio, "{");

  if (pretty) BIO_puts(bio, "\n");
  ...
  BIO_printf(bio, "\"alg\":%s\"%s\"", pretty ? " " : "", jwt_alg_str(jwt->alg));
...
static int jwt_encode_bio(jwt_t *jwt, BIO *out)
{
  ...
  if (jwt->alg == JWT_ALG_NONE) goto encode_bio_success;

  /* Now the signature. */
  ret = jwt_sign(jwt, b64, buf);
  if (ret) goto encode_bio_done;
