...
struct uuid
{
  ...
  template<typename ostream> inline friend ostream &operator<<(ostream &os, const uuid &self)
  {
...
namespace std {
template<> struct hash<sole::uuid> : public std::unary_function<sole::uuid, size_t>
{
public:
...
inline bool sole::uuid::operator==(const sole::uuid &other) const { return ab == other.ab && cd == other.cd; }
inline bool sole::uuid::operator!=(const sole::uuid &other) const { return !operator==(other); }
...
inline uint64_t rebase(const std::string &input, const std::string &basemap)
{
  ...
  for (size_t i = 1; i < limit; ++i) res = size * res + strpos(basemap, input[i]);
  return res;
}
...
inline std::string uuid::pretty() const
{
  ...
  if (version == 1) timestamp = timestamp - 0x01b21dd213814000ULL;// decrement Gregorian calendar
      ...
      $osx({
        ...
        $linux({
          ...
          $unix({
            ...
            if (gethostname(name, sizeof(name))) return $no("cannot get host name") false;
...
template<typename FN> void benchmark(const FN &fn, const std::string &name)
{
...
template<typename FN> void tests(const FN &fn)
{
  unsigned numtests = ~0;
  ...
  for (unsigned i = 0; i < numtests; ++i) {
    ...
    assert(
      all.find(my_uuid) == all.end() && "error: UUIDs just collided! is std::random_device a real random generator?");
    all.insert(my_uuid);
  }
}

template<typename FN> void verify(const FN &fn)
{
