...
unordered_map<string, string> parse_header_options(const string &options, bool form_encoded, char sep)
{
  ...
  if (old_pos != options.length()) { params.push_back(options.substr(old_pos, string::npos)); }
...
static unordered_map<string, string> parse_query_string(const string &query, bool form_encoded = false)
{
  ...
  if (old_pos != query.length()) { params.push_back(query.substr(old_pos, string::npos)); }
  ...
  while (!eoh && !ins->eof() && !ins->fail()) {
    ...
    if (line.empty() || ins->fail()) {
      ...
      if (name == "connection") {
        ...
        unordered_map<string, string> opts = parse_header_options(value, true, ',');
        if (opts.count("timeout") == 1) { _keep_alive_timeout = stoi(opts["timeout"]); }
      } else if (name == "content-length") {
        content_length = static_cast<size_t>(stoll(value));
      } else if (name == "transfer-encoding") {
        if (value == "chunked") { _chunked_transfer_in = true; }
      } else if (name == "host") {
        _host = value;
      }
    }
...
Connection::Connection(Server *server_p,
  std::istream *ins_p,
  std::ostream *os_p,
  const string &tmpdir_p,
  size_t buf_size,
  size_t buf_inc)
...
Connection::~Connection()
{
  ...
  if (_content != nullptr) { free(_content); }
...
int Connection::setupKeepAlive(int default_timeout)
{
  if (_keep_alive) {
    ...
    if (_keep_alive_timeout <= 0) { _keep_alive_timeout = default_timeout; }
...
string Connection::getParams(const std::string &name)
{
  ...
  if (get_params.count(name) == 1) { result = get_params[name]; }
...
vector<string> Connection::getParams()
{
  ...
  for (auto const &iterator : get_params) { names.push_back(iterator.first); }
...
string Connection::postParams(const std::string &name)
{
  ...
  if (post_params.count(name) == 1) { result = post_params[name]; }
...
vector<string> Connection::postParams()
{
  ...
  for (auto const &iterator : post_params) { names.push_back(iterator.first); }
...
string Connection::requestParams(const std::string &name)
{
  ...
  if (request_params.count(name) == 1) { result = request_params[name]; }
  return result;
}
//=============================================================================
...
vector<string> Connection::requestParams()
{
  ...
  for (auto const &iterator : request_params) { names.push_back(iterator.first); }
...
vector<string> Connection::header()
{
  ...
  for (auto const &iterator : header_in) { names.push_back(iterator.first); }
...
void Connection::flush()
{
  ...
  if ((outbuf != nullptr) && (outbuf_nbytes > 0)) {
    ...
    if (!_chunked_transfer_out) { _finished = true; }
  }
}
//=============================================================================
