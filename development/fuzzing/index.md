...
libFuzzer ships in Clang's compiler-rt. The build uses two registered LLVM toolchains, both sharing the same LLVM 19.1.7 download:

- On linux-x86_64, `@llvm_toolchain_fuzz` wins (libstdc++, mirroring today's `pkgs.llvmPackages_19.stdenv` override). The platform `//tools/fuzz:linux_x86_64_fuzz` carries the `fuzz_enabled` constraint the toolchain is gated on.
- On darwin-aarch64, the default `@llvm_toolchain` wins (Apple SDK + libc++, which is libFuzzer's ABI on darwin anyway). The platform `//tools/fuzz:darwin_aarch64_fuzz` activates the harness's `target_compatible_with = ["//tools/fuzz:fuzz_enabled"]` gate.
