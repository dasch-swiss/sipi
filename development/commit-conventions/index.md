...
A `fix:` corrects behavior that exists on `main` — a bug a deployer could hit today, or that a released version shipped. It earns a "Bug Fixes" changelog line and a patch bump precisely because deployers need to know.
...
The scope names the **module** the change belongs to. The canonical module list — which is also the scope vocabulary — lives in [`CONVENTIONS.md` § Module Layout](https://sipi.io/CONVENTIONS.md). Use one of these names, lowercase:

- **Module scopes:** `image`, `formats`, `metadata`, `iiifparser`, `handlers`, `shttps`, `cache`, `memory-budget`, `observability`, `logging`, `cli`, `ffi`, `lua`, `server-rs`, `cli-rs`
- **Test-layer scopes** (test-only changes spanning a whole layer): `e2e`, `approval`. A unit-test change takes the scope of the module under test.
...
- Scope is optional. Omit it for genuinely repo-wide changes (`chore: ...`, `ci: ...`).
- A commit that spans several modules may list them comma-separated: `refactor(cli-rs,server-rs): ...`.
- **If none of the enumerated scopes genuinely fits, ask the maintainer before inventing a new one.** New scopes are added to the canonical list in `CONVENTIONS.md` deliberately, not ad hoc.
...
1. Internal work (`build:`, `ci:`, `refactor:`, `docs:`, `chore:`, `test:`) is hidden from the changelog — squash aggressively.
