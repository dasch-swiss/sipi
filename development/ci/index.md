...
A commit without a valid Conventional Commit prefix will be invisible to release-please — it won't trigger a release or appear in the changelog. See [Commit Message Schema](../developing/#commit-message-schema) for the full format specification.
...
1. **Alpine Docker:** `docker run alpine:3.21` with source bind-mounted:
...
1. **Host (Ubuntu):** Verification and testing:
...
1. `validate-static / {arch}` builds, tests, and packages each architecture natively (same Docker-in-Ubuntu pattern as PR workflow).
1. `validate-docker` must pass.
1. `release-gate` requires `validate-docker` and `validate-static`.
1. Publish side effects run only after `release-gate` succeeds.

### Static artifact flow

Each architecture is built, tested, and packaged in its own `validate-static` job:
...
These targets mirror the `validate-static` CI job: Alpine 3.21 container, Zig toolchain, cmake build, and unit tests. They do **not** run e2e tests.
