...
1. **Size** — scale to requested dimensions
1. **Rotation** — rotate and/or mirror
1. **Quality** — color space conversion (color, gray, bitonal)
1. **Format** — encode to output format
...
The production HTTP server is the Rust axum shell (`src/server/rust` + `src/cli/rust`), which drives the C++ image engine over the FFI seam:
...
| **Metrics**           | hits, misses, evictions, skips, size, file count — the scalar counters cross the FFI seam via `SipiMetricsSnapshot` and export over OTLP (see below) |
| **API endpoints**     | None. There is no `/api/cache` route on the Rust shell; observe cache behaviour via the on-disk cache-dir file count                                 |
...
**Status: shipped.** Every fuzz target has two modes from one definition. By default (`rules_fuzzing`'s `replay` engine, no libFuzzer runtime) it is a corpus-replay regression test that builds and runs on every platform and rides along in the `//src/...` sweeps — including the sanitizer leg. `--config=fuzz` arms the real libFuzzer engine plus SanitizerCoverage on the Rust crate graph (for `parse_request_fuzz`) or the C++ codec handlers (for the four `format_handlers` decode harnesses); that mode is Linux-only and runs nightly.
...
The retired C++ libFuzzer harness (`//fuzz/handlers`) fuzzed the oracle-only C++ classifier and went away with the oracle. See [Fuzzing](https://sipi.io/development/fuzzing/index.md) for the shim seam, the corpus policy, and the nightly workflow.
...
| Prometheus metrics endpoint                                                       | N/A — removed                | —                                                                                                                                                                                 | OTLP replaces it on the Rust shell; there is no scrape endpoint                                                                                                                                          |
...
| Lua route handler errors                                                          | :white_check_mark:           | `server.rs::lua_route_error_handling`                                                                                                                                             |                                                                                                                                                                                                          |
...
| Lua JWT round-trip                                                                | :white_check_mark:           | `server.rs::lua_jwt_round_trip`                                                                                                                                                   |                                                                                                                                                                                                          |
| Lua UUID round-trip                                                               | :white_check_mark:           | `server.rs::lua_uuid_round_trip`                                                                                                                                                  | non-deterministic body                                                                                                                                                                                   |
| Lua `server.http` outbound                                                        | :white_check_mark:           | `server.rs::lua_http_client_error_handling`                                                                                                                                       |                                                                                                                                                                                                          |
| Lua error propagation to HTTP                                                     | :white_check_mark:           | `server.rs::lua_route_error_handling`                                                                                                                                             |                                                                                                                                                                                                          |
...
| CMYK→sRGB through IIIF pipeline                                                   | :white_check_mark:           | `iiif_compliance.rs::cmyk_through_iiif_pipeline`                                                                                                                                  |                                                                                                                                                                                                          |
| CIELab through IIIF pipeline                                                      | :white_check_mark:           | `iiif_compliance.rs::cielab_through_iiif_pipeline`                                                                                                                                |                                                                                                                                                                                                          |
| 16-bit depth through IIIF pipeline                                                | :white_check_mark:           | `iiif_compliance.rs::bit16_through_iiif_pipeline`                                                                                                                                 |                                                                                                                                                                                                          |
| Progressive JPEG handling                                                         | :white_check_mark:           | `iiif_compliance.rs::progressive_jpeg_input`                                                                                                                                      |                                                                                                                                                                                                          |
...
| Watermark application via HTTP                                                    | :white_check_mark:           | `server.rs::watermark_applied_via_http`                                                                                                                                           |                                                                                                                                                                                                          |
| Restrict + watermark combined                                                     | :white_check_mark:           | `server.rs::restrict_plus_watermark`                                                                                                                                              |                                                                                                                                                                                                          |
...
Memory leaks and undefined behavior are not a separate pyramid layer but a **build variant** that runs existing tests with compiler instrumentation. This is critical for sipi's long-running C++ image engine, where leaks accumulate over the process lifetime.
...
| Component microbenchmarks | Parse / decode / process / encode tiers, before/after comparisons for hot-path PRs | Google Benchmark (`just bench`) | **Done** — local dev loop, never a CI gate; see [Benchmarking](https://sipi.io/development/benchmarking/index.md) |
...
1. Run tests: `just bazel-test-e2e` (pending snapshots are written as `*.snap.new` next to the existing `*.snap`)
1. Review the diff between each `.snap` and `.snap.new` (a regular `git diff` works, or open both in your editor)
1. For intentional changes, replace: `mv path/to/foo.snap.new path/to/foo.snap`
1. For regressions, delete the `.snap.new` and fix the code
1. Commit updated `.snap` files

For interactive review, install `cargo-insta` ad-hoc — it operates on `.snap.new` files and doesn't need the dev-shell rust toolchain:
...
| Fuzz mutation loop          | `just bazel-build-fuzz` + the runner-local loop (`just fuzz` locally)                    | nightly (`fuzz.yml`), Linux only | libFuzzer + SanitizerCoverage on the Rust parser via the `//src/iiifparser/fuzz` shim and on the C++ codec handlers via `//src/format_handlers/fuzz`; see [Fuzzing](https://sipi.io/development/fuzzing/index.md) |
...
1. **During migration:** Write Rust unit tests (`#[test]`, `proptest`) for the new implementation
1. **After migration:** Existing e2e tests validate the Rust implementation matches C++ behavior
1. **Cleanup:** Remove corresponding C++ unit tests (they tested the old implementation)

The `insta` golden baselines are critical — they capture the Rust shell's exact response output and detect any implementation drift.
...
(Component microbenchmarks — formerly listed here as a post-Rust-migration `criterion` aspiration — exist today as the C++ Google Benchmark suite; see [Benchmarking](https://sipi.io/development/benchmarking/index.md). If/when the codec migrates to Rust, the discipline carries over to `criterion` unchanged.)
