...
- Do **not** frame the Rust shell relative to the removed C++ server / oracle / transport ("matches the oracle", "the transport's X", "at the cutover", "reconstructs shttps' Y"). Referencing the C++ **engine** (the production FFI callee) is fine: that is what the shell calls into. The general rule is [`.claude/rules/comments.md`](.claude/rules/comments.md).
...
`CONVENTIONS.md` / `CONTEXT.md` / `UBIQUITOUS_LANGUAGE.md`, `.claude/rules/comments.md` and the ADRs; a doc
