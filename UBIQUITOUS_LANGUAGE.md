...
| **Metrics** | The instrumentation surface. The engine's Prometheus singleton in `observability/metrics.{h,cpp}` holds ~25 metrics: counters (cache hits/misses/evictions/skips, image-too-large, client-disconnects, memory-alloc-failures, rate-limit decisions, decode-memory decisions, rejected-connections), gauges (waiting-connections, cache size/files, decode-memory-budget/used, rate-limit-clients-tracked, build-info), histograms (request duration, decode-memory estimate). `GET /metrics` (`text/plain; version=0.0.4`) serialises it on the retained C++ server only — oracle-only, never deployed. **Production is OTLP:** scalars cross the seam as `SipiMetricsSnapshot` and re-register as OTel observable instruments in `server-rs/src/metrics.rs`; distributions are recorded shell-side as OTel histograms (`http.server.request.duration`, `sipi.decode_memory.estimate_bytes`); `build_info` becomes the resource attributes `service.version` + `vcs.ref.head.revision`. Singleton (the `shttps/transport/Server.cpp → Metrics::instance()` consumption is documented as the first known layering leak in `CONTEXT.md`). | telemetry, prom registry |
