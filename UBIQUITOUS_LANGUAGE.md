...
| **Image shape** | The intrinsic shape of a source image: `(img_w, img_h, tile_w, tile_h, clevels, numpages, nc, bps)`. Read by a format handler from a *Service File* via `read_shape()` (formerly `getDim()`). Stored in the *Essentials packet* so server-mode shape lookup can read at a known offset rather than parsing the codestream / TIFF tags. Per ADR-0004. | size record, dimensions |
...
| Term | Definition | Aliases to avoid |
| --- | --- | --- |
| **File role** | The intentional purpose of an image file in the SIPI workflow: *Preservation File*, *Service File*, or *Access File*. Per ADR-0009. The role is established at creation time by the operator's explicit CLI invocation (see ADR-0010) and recorded in the file's metadata: presence of an *Essentials packet* discriminates a Service File from an Access File; the future preservation-metadata schema (ADR-0012) discriminates a Preservation File. Role is **orthogonal to file format** — a JP2 may be a Service File (with packet) or an Access File (without). | master, file kind |
...
| **Pixel checksum** | A checksum (e.g. SHA-256) over the *uncompressed* pixel values, stored in the *Essentials packet* to verify that a format conversion did not alter image content. Computed on the post-transformation pixel buffer by the master-creation orchestrator (per ADR-0010). | data checksum, content hash |
| **Master-creation orchestrator** | The dispatcher invoked by `sipi convert service-file` (and future `sipi convert preservation-file`) that builds the role-establishing metadata for the output file: reads the source via `readSource`, applies transformations, computes the *Pixel checksum* on the post-transformation pixel buffer, assembles `EssentialsFields` (or future preservation-metadata structure) from observed source + computed hash + current `SipiImage` state, and hands to the format-handler writer with `master_mode` set. Per ADR-0010. Lives in `src/cli/service_file_orchestrator.{h,cpp}` (Service File path; Preservation File path future). | service-master orchestrator |
...
| **Operating mode** | Umbrella for the two ways SIPI runs: *CLI mode* and *Server mode*. The asymmetry between which format handler dominates read vs. write is architecturally load-bearing for the Service-File-format fast path. The C++ enum `SipiMode { CLI, Server }` is the runtime tag (used by Sentry flush behaviour, error-reporting paths, and conversion-pipeline asymmetries). | mode |
| **CLI mode** | One-shot invocation. Verb-noun subcommand surface (per ADR-0010): generic verbs (`convert`, `verify`, `query`, `compare`) are ImageMagick-style utilities usable by anyone; DSP-specific verbs (`convert service-file`, `convert access-file`, future `convert preservation-file`; `verify service-file`, `verify access-file`, future `verify preservation-file`) enforce preservation-chain semantics. **Characteristics:** blocking Sentry flush, optional `--json` *CLI report* on stdout, `set_cli_mode(true)` redirects logs to stderr. Code-level boundary: `src/cli/cli_mode.{h,cpp}`. | one-shot mode |
| **Server mode** | Long-running HTTP server. Reads *Service Files* in *Service File format* from *Image root*; writes *Access Files* (via the IIIF pipeline) to the *Cache* and HTTP response. The hot path for *Service File*-format shape reads. Conceptually `convert access-file` over HTTP — the IIIF server applies IIIF Image API parameters to a Service File and emits an Access File response. **Characteristics:** *Server config* value type at construction, three Throttling sub-policies at the post-cache gate, *Connection metrics adapter* installed on shttps at startup, non-blocking Sentry flush. Code-level boundary: `src/cli/server_mode.{h,cpp}` (`run_server`). | daemon mode, http mode |
...
- **File roles** form a chain: **Preservation File** (long-term bit-level preservation) → **Service File** (mezzanine baseline read by SIPI server) → **Access File** (end-user delivery). Each step is intentional (per ADR-0010). The role is established by the operator's CLI invocation at creation time, never inferred from format. **Service File** ⊂ **Service File format** is what server mode reads; **Preservation File** ⊂ **Preservation File format** is the OAIS preservation copy.
...
> **Maintainer:** Only on intentional write — specifically `sipi convert service-file`, which is the master-creation orchestrator's entry point. Plain `sipi convert` (the generic ImageMagick-style verb) doesn't emit one, even if the output happens to be a *Service File format* like JP2 — that output would be an *Access File* by intent. On read, we extract any existing essentials packet — that's how we recover an ICC profile when a JPEG2000 source couldn't natively embed one, and how server mode answers shape queries with one **range GET** instead of parsing the full codestream. The **corruption tripwire** in `readSource` logs ERROR if the packet's checksum doesn't match the recomputed pixel hash — that's the passive integrity signal; the active deliberate check is `sipi verify service-file <file>`.
...
- **"master"** was used unqualified for both the SIPI-served file and the preservation copy, and the IIIF-derivative concept was left implicit. **Resolved** (per ADR-0009): three-tier *File role* taxonomy — **Preservation File** (long-term bit-level preservation; was: Archival master), **Service File** (mezzanine baseline read by SIPI server; was: Service master), **Access File** (end-user delivery; previously implicit). Each role has its own format set; the role discriminator is the *Essentials packet*'s presence (Service File) or the future preservation-metadata schema (Preservation File), not the file format. Role is established by the operator's intentional CLI invocation (per ADR-0010), never inferred. The "master" terminology is retired with this rename.
