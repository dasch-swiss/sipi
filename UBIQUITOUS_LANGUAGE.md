...
> **Current vs. target.** The tables below describe the code **as it exists today** — every path and symbol named resolves in the tree. One *current* ADR commits to a structure that is not yet built: a typed read-path `Input source` (ADR-0006, ADR-0004). It is collected under [Target shape (not yet built)](#target-shape-not-yet-built) at the end, never mixed into the current-state definitions. Note: the deployed server is the Rust shell; the C++ `SipiHttpServer` transport was removed (ADR-0020), and the older ADR-0001 C++-refactor targets for it (a composed `Sipi::Server`, `route_handlers/` / `permission/` packages) never happened — ADR-0013 repointed the rewrite at Rust.

## Resources and identification

| Term | Definition | Aliases to avoid |
| --- | --- | --- |
| **Image** | A pixel-bearing artefact processed through the IIIF pipeline (region, size, rotation, quality, format). The domain-level term. The *code-level* class is `Sipi::SipiImage` (`src/image/cpp/SipiImage.h`): a value type carrying geometry + photometric + RAII pixel buffer + metadata composite, with decode/encode orchestration (`read`/`readSource`/`write`) and a pixel/metadata mutator surface. Image-processing behaviour (`crop`, `scale`, `rotate`, `convertToIcc`, `add_watermark`, …) is not on the class — see *Image processing* below (ADR-0007). | resource, media, asset |
...
| Term | Definition | Aliases to avoid |
| --- | --- | --- |
| **Image processing** | Umbrella term for SIPI's pixel operations over an *Image*: crop, scale, rotate, colour conversion, channel ops, bit-depth reduction, dithering, watermark application, comparison, arithmetic. Free functions over `SipiImage &` in `Sipi::processing::` (`src/image_processing/cpp/processing.h`), extracted off the `Sipi::SipiImage` class per ADR-0007. `image_processing` depends one-way on the `image` value type — never the reverse, so a facade method on `SipiImage` is not an option. | (none) |
| **Image shape** | The intrinsic shape of a source image: `(img_w, img_h, tile_w, tile_h, clevels, numpages, nc, bps)`. Read by a format handler from a *Service File* via `SipiIO::read_shape()` (`src/image/cpp/SipiIO.h`; the file-based, no-full-decode probe — distinct from `SipiImage::getDim()`, which returns the dims of an already-loaded in-memory image). Stored in the *Essentials packet* so server-mode shape lookup can read at a known offset rather than parsing the codestream / TIFF tags. Per ADR-0004. | size record, dimensions |
| **Watermark** | Overlay image applied to an *Image* before serving when a `restrict` *Permission* carries a watermark path. Applied via the free function `Sipi::processing::add_watermark(SipiImage &img, const std::string &wmfilename)` (`src/image_processing/cpp/compose.cpp`), which loads the watermark file through `Sipi::read_watermark`, defined in the TIFF *Format handler* (`src/format_handlers/cpp/SipiIOTiff.cpp`). Watermark presence extends the *Canonical URL* into the *Cache key* (`/0` or `/1` suffix). | overlay |
...
| **Output sink** | Typed sum type for write-path I/O destinations: `using OutputSink = std::variant<FilePath, CallbackSink, TeeSink>` in `src/format_handlers/cpp/output_sink.h`. Format-handler `write()` API (`src/image/cpp/SipiIO.h`) takes one, replacing magic-string sentinels (`"-"` for stdout, `"HTTP"` for HTTP server). `CallbackSink` carries opaque write/finalize callbacks, so `src/format_handlers/` does not depend on `shttps/`. Per ADR-0006. | (none) |
| **Tee sink** | Composition primitive in the *Output sink* variant: `TeeSink { std::vector<OutputSink> sinks; }` (`src/format_handlers/cpp/output_sink.h`) broadcasts each output chunk to multiple sub-sinks. Preserves SIPI's existing dual-write optimization (encoder writes simultaneously to HTTP socket + cache file). Generalises to write-through to S3 / other sinks. Per ADR-0006. | (none) |
...
schema per ADR-0012 for Preservation Files). **Documented exception:**
the upload Lua route (`SipiImage:write` on an uploaded file) also
produces a Service File — it stamps the Essentials packet without a
`sipi convert` invocation. It is the only non-`convert` producer. The stage is orthogonal
...
| **Kill semantics** | What happens when a request VM exceeds its limits (timeout or memory, `KillReason`): before the response head commits, the request becomes a 500 with a generic body; after commit, the body stream is aborted so clients never see a clean EOF; a killed preflight is never written to the *Preflight cache*. Kills are counted in `sipi.lua.kills` (rendered `sipi_lua_kills_total{reason}`) and logged as one structured line. | script abort, watchdog |
| **Bytecode cache** | The in-memory compiled-chunk cache (`BytecodeCache`, `src/scripting/rust/runtime.rs`) covering the init script, route scripts, and `require`d modules: compiled once, loaded per VM as binary chunks (debug info kept), invalidated by mtime+size so script edits take effect immediately. Distinct from *Cache* (the file-based LRU of representations) and *Preflight cache* (the permission-decision cache). | script cache, chunk cache |
...
| Term | Definition | Aliases to avoid |
| --- | --- | --- |
| **Throttling** | Umbrella term for SIPI's load-driven request-rejection policies. Comprises two sub-policies: *Admission* (the two-partition thread pool) and *Decode memory budget* (the full partition's decode-RAM cap). Admission is a shell-side pool (`src/throttling/rust`, pre-dispatch); the memory budget is engine-side (`src/throttling/cpp`, post-cache). | backpressure (technically denotes upstream feedback flow control, which SIPI does not do — it rejects with HTTP 503/400), flow control |
| **Admission** | The shell-side pool that classifies each request into the *Tile partition* or *Full partition* and bounds concurrency with two semaphores: a global pool (sized to the worker count) and a full sub-pool (`full_max = nthreads − tile_min`) that hard-caps concurrent full-image decodes. Tiles take a global permit only and burst into idle capacity; a full takes its sub-pool permit first, then the global permit, so tiles always keep a `≥ tile_min` floor. Shed requests get HTTP 503 + `Retry-After`. `struct Admission` + `AdmissionKind` + `AdmissionMode` in `//src/throttling/rust:admission`; `AppState` owns it, `routes.rs` holds the call sites. Distinct from *Permission* (the Lua access decision) — Admission is load-based capacity, Permission is per-request authorization. | admission control (use *Admission*), backpressure |
| **Tile partition / Full partition** | The two request classes *Admission* routes to. **Tile**: cheap requests — viewer tiles, `info.json`/`knora.json`, the `/file` byte stream — guaranteed a thread floor (`tile_min`) and free to burst; they bypass the decode memory budget. **Full**: expensive full-image decodes (estimated peak ≥ `large_decode_threshold_bytes`) plus Lua routes and docroot scripts (a script's cost is unknowable up front), hard-capped in both threads (`full_max`) and decode RAM. Classification is coarse shell-side (from the IIIF params) and precise engine-side (from `estimate_peak_memory`); the two are tracked and their disagreement is observable. *Tile lane / full lane* are accepted informal synonyms (the design is "two-lane"); *partition* is the precise term. | tile class / full class |
| **Admission mode** | `basic` \| `advanced` (there is no "off"; default `basic`). A two-tier model: the *basic tier* (the global CPU/thread concurrency cap) is always enforced; the *advanced tier* (the full thread cap plus the memory budget and two-lane load-shedding) is enforced only under `advanced`. `basic` shadow-counts what the advanced tier would shed so the operator can size the full partition before switching; `advanced` applies the full thread cap and the memory budget and sheds with 503/413. An unrecognized value is not a startup error — it falls back to `basic`. Single-sourced: the engine parses it and the shell reads it back over the seam, so both run the same mode. `enum AdmissionMode { Basic, Advanced }` (Rust) / `enum class AdmissionMode { BASIC, ADVANCED }` (C++). | shadow mode (use *basic*), monitor / enforce (legacy value strings, use *basic* / *advanced*) |
| **Decode memory budget** | The *Full partition*'s cap on decode RAM: a process-wide, lock-free accounting of memory committed to in-flight full-lane decodes, with an RAII guard. Tile decodes bypass it. The cap is `full_mem = memory_limit × (1 − tiles_memory_ratio)`. Rejects with HTTP 503 + `Retry-After` when currently exhausted, or HTTP 413 (no `Retry-After`) when a single request's estimate alone exceeds the whole budget. `class Sipi::SipiMemoryBudget` + `MemoryBudgetGuard` + `enum class AdmissionMode { BASIC, ADVANCED }` in `src/throttling/cpp/SipiMemoryBudget.{h,cpp}`; the peak-memory helper is `estimate_peak_memory()` in `src/throttling/cpp/SipiPeakMemory.h`. The acquire site is `src/ffi/cpp/serve_image.cpp`. | memory budget, decode budget |
| **Cache** | A file-based LRU of generated representations, keyed by *Cache key*, with dual-limit eviction (total size **and** file count) and crash recovery. `class SipiCache` in `src/cache/cpp/SipiCache.{h,cpp}`. Cache state is exposed exclusively through *Metrics*, not through Lua bindings. Cache-hit responses **bypass the memory-budget and output-size policies entirely** (per ADR-0008): no memory-budget acquire, no output-size check. (A cache hit is still classified and admitted by the shell pre-dispatch, so a "full" cache hit briefly holds a full-partition permit — throttled by class, released fast; accepted.) | response cache, output cache |
| **Preflight cache** | A burst-coalescing cache of the Lua `pre_flight` access decision (the *Permission* + resolved path), so a repeated request serves the recorded decision instead of re-running the hook — a deep-zoom viewer fires `pre_flight` once per tile, all sharing one *Identifier*. Distinct from the *Cache* (which stores generated representations, not access decisions). Opt-in (`--preflight-cache-ttl <secs>` / `SIPI_PREFLIGHT_CACHE_TTL`, default 0 = off); the TTL bounds staleness on a permission change. `mod preflight_cache` in `src/server/rust`. Correct **only** for a hook whose decision is a pure function of its **Preflight cache key** = `(prefix, identifier, Cookie, Authorization)`; a hook that reads any unkeyed request field (other headers, host, client IP) may be served a wrong cached decision. The preflight request view is query-free by construction (`uri` is the path only; no get/post/request params) and the key never adds the raw query string — that would make a query-string cache-buster an unbounded amplifier against dsp-api. | pre-flight cache, auth cache |
| **Cache pin** | Keeping a cache file in use so it is not evicted while a representation is being served. Today this is manual and non-RAII: `SipiCache::check(origpath, canonical, /*block_file=*/true)` (`src/cache/cpp/SipiCache.h`) inserts into the `blocked_files` map, and `SipiCache::deblock(res)` removes it — the caller must remember the `deblock`. (Wrapping this in an RAII `BlockedScope` was an ADR-0001 target that lapsed under ADR-0013.) | cache lock |
| **Client abort** | An HTTP response write that fails because the peer is gone (FIN, RST, or write timeout). Surfaces in code as `ErrorCode::kClientAbort` (`src/error/cpp/SipiValueError.h`), returned when `shttps::OUTPUT_WRITE_FAIL` is thrown from a socket write. `policy_for(ErrorCode::kClientAbort)` is `SentryPolicy::kSkip` with the `kClientDisconnected` metric hint: logged at info, **not** captured to Sentry — these are peer-side events, not server faults. | broken pipe error, peer disconnect error |
...
- **Throttling** = **Admission** (shell-side two-partition pool, pre-dispatch) + **Decode memory budget** (engine-side at the post-cache gate). The **Cache** short-circuits before the engine-side policy.
...
> **Maintainer:** Two places, actually. Before dispatch the shell's **Admission** pool classifies the request tile-or-full and admits it (or sheds 503). Then, if the cache misses, the engine's **decode memory budget** fires at the post-cache gate: it returns 503 if admitting the decode would push us over, or 413 if the request's estimate alone exceeds the whole budget. If the cache hits, we serve and the gate never runs. Only after the budget passes does the request reach the **format handler** and the **codec**.
