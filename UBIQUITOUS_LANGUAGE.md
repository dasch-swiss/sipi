...
| **Essentials packet** | The SIPI-specific record embedded in image headers. **Role:** shape + S3-access file-structure index. **Contents:** *Image shape* (8 fields), per-format file-structure offsets (TIFF: per-IFD offset/size; JP2: codestream + per-resolution offsets), ICC profile (when destination format cannot embed natively), original filename / mimetype / hash type / *Pixel checksum*. **Wire format:** versioned CBOR per ADR-0005 (legacy: pipe-delimited text, brittle). **Position:** a known fixed prefix offset in the file (TIFF tag in the first IFD; JP2 UUID box near the start) so SIPI can fetch with one *Range GET* of the prefix. C++ class: `Essentials`. | preservation packet, sipi metadata |
