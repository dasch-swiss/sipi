...
| Term | Definition | Aliases to avoid |
| --- | --- | --- |
| **Throttling** | Umbrella term for SIPI's load-driven request-rejection policies. Comprises two sub-policies: *Decode memory budget* (process-wide instantaneous decode RAM), *Output size guard* (intrinsic max-output-pixels ceiling). Both fire at one post-cache gate site per ADR-0008. Lives in `src/throttling/`. | backpressure (technically denotes upstream feedback flow control, which SIPI does not do — it rejects with HTTP 503/400), flow control, admission control (collides with *Permission*) |
...
| **Cache** | A file-based LRU of generated representations, keyed by *Cache key*, with dual-limit eviction (total size **and** file count) and crash recovery. Cache state is exposed exclusively through *Metrics* (Prometheus), not through Lua bindings. Cache-hit responses **bypass both Throttling policies entirely** (per ADR-0008): no memory-budget acquire, no output-size check. | response cache, output cache |
...
| **Metrics** | The instrumentation surface. The engine's Prometheus singleton in `observability/metrics.{h,cpp}` holds ~25 metrics: counters (cache hits/misses/evictions/skips, image-too-large, client-disconnects, memory-alloc-failures, decode-memory decisions, rejected-connections), gauges (waiting-connections, cache size/files, decode-memory-budget/used, build-info), histograms (request duration, decode-memory estimate). `GET /metrics` (`text/plain; version=0.0.4`) serialises it on the retained C++ server only — oracle-only, never deployed. **Production is OTLP:** scalars cross the seam as `SipiMetricsSnapshot` and re-register as OTel observable instruments in `server-rs/src/metrics.rs`; distributions are recorded shell-side as OTel histograms (`http.server.request.duration`, `sipi.decode_memory.estimate_bytes`); `build_info` becomes the resource attributes `service.version` + `vcs.ref.head.revision`. Singleton (the `shttps/transport/Server.cpp → Metrics::instance()` consumption is documented as the first known layering leak in `CONTEXT.md`). | telemetry, prom registry |
...
| Term | Definition | Aliases to avoid |
| --- | --- | --- |
| **Server (Sipi side)** | `Sipi::Server` in `src/server/`, *composing* (not inheriting) `shttps::Server`, per the strangler-fig direction in ADR-0001. It is part of the retained C++ transport that now serves as the differential oracle; the Rust shell (`//src/cli-rs:sipi`) is the deployed server. Paper-thin (~180 lines): constructor, `run()` (realpath imgroot validation + `register_routes()` + delegation), runtime-resource owners. | (none) |
