...
> **Current vs. target.** The tables below describe the code **as it exists today** — every path and symbol named resolves in the tree. Two *current* ADRs commit to a structure that is not yet built: a free-function image-processing module (ADR-0007) and a typed read-path `Input source` (ADR-0006, ADR-0004). Those are collected under [Target shape (not yet built)](#target-shape-not-yet-built) at the end, never mixed into the current-state definitions. Note: the C++ transport (`SipiHttpServer` and its route handlers) is the retained differential **oracle**, not the deployed server (the Rust shell is); its internal structure is described as-is, and the older ADR-0001 refactor targets for it (a composed `Sipi::Server`, `route_handlers/` / `permission/` packages) lapsed when ADR-0013 repointed the rewrite at Rust.
...
| **C++ route handler** | A `shttps::RequestHandler` callback registered at server startup. Today these are `static` free functions in `src/SipiHttpServer.cpp` (`iiif_handler`, `health_handler`, `metrics_handler`, `favicon_handler`; file-serving uses the framework's `shttps::file_handler`), registered by `SipiHttpServer::run()` calling the inherited `add_route(method, path, handler, handler_data)` once per route. (Extracting them into a `route_handlers/` package with a `register_routes()` free function was an ADR-0001 target that lapsed under ADR-0013.) Note: `shttps::RequestHandler` remains the framework type; *C++ route handler* is the SIPI-side domain term for instances of it. | (none) |
...
| Term | Definition | Aliases to avoid |
| --- | --- | --- |
| **Server (Sipi side)** | `class SipiHttpServer` (`src/SipiHttpServer.{h,cpp}`), which **inherits** `shttps::Server` (`src/SipiHttpServer.h:37`). It is the retained C++ transport that now serves as the differential oracle; the Rust shell (`//src/cli-rs:sipi`) is the deployed server. (The superseded ADR-0001 once targeted a composed `Sipi::Server` in `src/server/`; that C++ refactor lapsed when ADR-0013 repointed the rewrite at Rust.) | (none) |
...
| **Server context** | The server-scope dependency bundle a *C++ route handler* reads. In the oracle transport this is the god-pointer: `auto *serv = static_cast<SipiHttpServer *>(user_data)` (`src/SipiHttpServer.cpp`, e.g. `iiif_handler`), the `user_data` passed to the inherited `add_route()`. (A typed `ServerContext` bundle was an ADR-0001 refactor target that lapsed under ADR-0013.) | route context |
...
| **Server mode** | Long-running HTTP server. Reads *Service Files* in *Service File format* from *Image root*; writes *Access Files* (via the IIIF pipeline) to the *Cache* and HTTP response. The hot path for *Service File*-format shape reads. Conceptually `convert access-file` over HTTP — the IIIF server applies IIIF Image API parameters to a Service File and emits an Access File response. **Characteristics:** two Throttling sub-policies at the post-cache gate, *Connection metrics adapter* installed on shttps at startup, handled image errors reported to Sentry via the FFI seam (see *Sentry context*). Code-level boundary: the `run_server` lambda in `src/cli/cli_app.cpp`. | daemon mode, http mode |
...
repointed the rewrite at Rust and the C++ transport is now the differential oracle,
so those refactors are no longer intended.
