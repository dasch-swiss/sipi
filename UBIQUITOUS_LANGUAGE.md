...
| Term | Definition | Aliases to avoid |
| --- | --- | --- |
| **Throttling** | Umbrella term for SIPI's load-driven request-rejection policies. Comprises three sub-policies: *Admission* (the two-partition thread pool), *Decode memory budget* (the full partition's decode-RAM cap), and *Output size guard* (intrinsic max-output-pixels ceiling). Admission is a shell-side pool (`src/throttling/rust`, pre-dispatch); the memory budget is engine-side (`src/throttling/cpp`, post-cache); the output-size guard is inline at the `src/ffi/serve_image.cpp` gate. | backpressure (technically denotes upstream feedback flow control, which SIPI does not do — it rejects with HTTP 503/400), flow control |
...
- **Throttling** = **Admission** (shell-side two-partition pool, pre-dispatch) + **Decode memory budget** + **Output size guard** (both engine-side at the post-cache gate). The **Cache** short-circuits before the two engine-side policies.
